...
export async function isPrototypeRoot(dir: string): Promise<boolean> {
  // 1. mozole.config.json check
  const configPath = path.join(dir, "mozole.config.json");
  if (await exists(configPath)) {
    try {
      const config = JSON.parse(await readFile(configPath, "utf8"));
      if (config.projectsDir === "projects" || config.type === "prototype") {
        return true;
      }
  ...
  // 2. package.json declaring prototype or project workspaces
  ...
  if (await exists(manifestPath)) {
    try {
      ...
      const workspaces: unknown = Array.isArray(pkg.workspaces)
        ? pkg.workspaces
      ...
      if (
        ...
        workspaces.some(
          (w) =>
            typeof w === "string" &&
            (w === "projects/*" || w === "projects" || w.startsWith("projects/")),
        )
      ) {
        const projectsPath = path.join(dir, "projects");
        if ((await exists(projectsPath)) && (await isDirectory(projectsPath))) {
  ...
  // 3. PROTOTYPE.md generated by prototypeInit alongside projects/ directory
  const protoDocPath = path.join(dir, "PROTOTYPE.md");
  if (await exists(protoDocPath)) {
    const projectsPath = path.join(dir, "projects");
    if ((await exists(projectsPath)) && (await isDirectory(projectsPath))) {
      return true;
    }
